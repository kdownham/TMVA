...
void MultiApplication(TString weightname, bool makeHists = false, bool makeDatacard = true){
   ...
   for (auto f : files){
	...
	float cut2_tempSQw = 0.;
        ...
        for (Long64_t ievt=0; ievt<tree->GetEntries(); ievt++){
		 ...
		 bool WWZ_SR_1 = ( WWZ_score > 0.97 && ZH_score < 0.02 );
		 bool WWZ_SR_2 = ( WWZ_score > 0.94 && WWZ_score < 0.97 && ZH_score < 0.03 );
		 bool ZH_SR_1  = ( WWZ_score < 0.06 && WWZ_score > 0.01 && ZH_score > 0.93 );
		 bool ZH_SR_2  = ( WWZ_score < 0.15 && WWZ_score > 0.06 && ZH_score > 0.85 && ZH_score < 0.93 );
   ...
   float cut1 = cut1_yields[0] + cut1_yields[1] + cut1_yields[2] + cut1_yields[3];
   float cut2 = cut2_yields[0] + cut2_yields[1] + cut2_yields[2] + cut2_yields[3];
   float cut3 = cut3_yields[0] + cut3_yields[1] + cut3_yields[2] + cut3_yields[3];
   float cut4 = cut4_yields[0] + cut4_yields[1] + cut4_yields[2] + cut4_yields[3];

   if (makeDatacard){
       std::cout << "imax   4 number of bins" << std::endl;
       std::cout << "jmax   " << cut1_yields.size() - 1 << " number of processes minus 1" << std::endl;
       std::cout << "kmax   * number of nuisance parameters" << std::endl;
       std::cout << "-------------------------------------------------------------------" << std::endl;
       std::cout << "-------------------------------------------------------------------" << std::endl;
       std::cout << "bin                   SR1           SR2            SR3           SR4         " << std::endl;
       std::cout << "observation           " << cut1 << "   " << cut2 << "   "  << cut3 << "   " << cut4 << std::endl;
       std::cout << "-------------------------------------------------------------------" << std::endl;
       std::cout << "bin      SR1  SR1  SR1  SR1  SR2  SR2  SR2  SR2  SR3  SR3  SR3  SR3  SR4  SR4  SR4  SR4" << std::endl;
       std::cout << "process  WWZ  ZH   TTZ  ZZ   WWZ  ZH   TTZ  ZZ   WWZ  ZH   TTZ  ZZ   WWZ  ZH   TTZ  ZZ " << std::endl;
       std::cout << "process  -1   0    1    2    -1   0    1    2    -1   0    1    2    -1   0    1    2  " << std::endl;
       std::cout << "rate     "<< cut1_yields[0] << " " << cut1_yields[1] << " " << cut1_yields[2] << " " << cut1_yields[3] << " " << cut2_yields[0] << " " << cut2_yields[1] << " " << cut2_yields[2] << " " << cut2_yields[3] << " " << cut3_yields[0] << " " << cut3_yields[1] << " " << cut3_yields[2] << " " << cut3_yields[3]  << " " << cut4_yields[0] << " " << cut4_yields[1] << " " << cut4_yields[2] << " " << cut4_yields[3]  << std::endl;
       std::cout << "-------------------------------------------------------------------" << std::endl;
   }
