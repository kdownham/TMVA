...
int MultiClassification(TString weightName){
    ...
    Use["BDT_nom"]         = 1; // uses Gradient Boost ( values determined to prevent overtraining )
    Use["BDT_def"]         = 1; // Gradient boost (default values for all parameters)
    Use["BDT_400trees"]    = 1; // Gradient boost (Fewer trees)
    Use["BDT_NS10"]        = 1; // Gradient boost (larger node size)
    Use["BDT_Depth2"]      = 1; // Gradient boost (Shallower depth)
    //=====================================================================
    // 
    ...
    TString dir = "/home/users/kdownham/Triboson/VVVNanoLooper/analysis/output_081723_metUpdate/Run2/";
    ...
    dataloader->AddVariable("STLepHad", "#Sigma_{lep,had,MET} p_{T}", "GeV", 'F');
    ...
    dataloader->PrepareTrainingAndTestTree( cut_all,
    					  "nTrain_WWZ=10000:nTrain_ZH=10000:nTrain_ZZ=10000SplitMode=Random:NormMode=NumEvents:!V" );
    //dataloader->PrepareTrainingAndTestTree( cut_all,
    //					  "SplitMode=Random:NormMode=NumEvents:!V" );
   

    // Boosted Decision Trees (BDTs)
    if (Use["BDT_nom"]) // Gradient Boost
        factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDT_nom",
    ...
    if (Use["BDT_def"])
	factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDT_def",
			   "!H:!V:NTrees=800:MinNodeSize=5%:MaxDepth=3:BoostType=Grad:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=SDivSqrtSPlusB:nCuts=20:IgnoreNegWeightsInTraining=True" );

    if (Use["BDT_400trees"])
        factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDT_400trees",
                           "!H:!V:NTrees=400:MinNodeSize=5%:MaxDepth=3:BoostType=Grad:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=SDivSqrtSPlusB:nCuts=20:IgnoreNegWeightsInTraining=True" );

    if (Use["BDT_NS10"])
        factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDT_NS10",
                           "!H:!V:NTrees=800:MinNodeSize=10%:MaxDepth=3:BoostType=Grad:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=SDivSqrtSPlusB:nCuts=20:IgnoreNegWeightsInTraining=True" );

    if (Use["BDT_Depth2"])
        factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDT_Depth2",
                           "!H:!V:NTrees=800:MinNodeSize=5%:MaxDepth=2:BoostType=Grad:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=SDivSqrtSPlusB:nCuts=20:IgnoreNegWeightsInTraining=True" );
