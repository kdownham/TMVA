...
void plotHists(bool draw2Donly=false, bool makeSRs=true){
     ...
     for (int i=0; i<t->GetEntries(); i++){
	  t->GetEntry(i);
          ...
          bool wwz_SR1 = ( wwz_score > 0.7 && zh_score < -0.6 );
          bool wwz_SR2 = ( wwz_score < 0.7 && wwz_score > 0.4 && zh_score < -0.6 );
          bool zh_SR1  = ( wwz_score > 0.5 && zh_score > 0.8 );
          bool zh_SR2  = ( wwz_score < 0.5 && wwz_score > 0.0 && zh_score > 0.8 );
     ...
     legend->AddEntry(histBDT_TTZ,"t#bar{t}Z","f");
