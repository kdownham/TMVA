...
void plotHists(bool draw2Donly=true, bool makeSRs=true, bool drawSRs=false, bool makeDatacard=true, bool makeLatex=true){
     ...
     for (int i=0; i<t->GetEntries(); i++){
	  ...
	  //bool wwz_SR1 = ( wwz_score > 0.9 && zh_score > 0.8 );
          ...
          ////bool wwz_SR1 = ( wwz_score > 0.30 && zh_score < -0.60 );
          ////bool wwz_SR2 = ( wwz_score > 0.80 && zh_score > -0.60 && zh_score < 0.90 );
          ////bool zh_SR1  = ( wwz_score > 0.7 && zh_score > 0.90 );
          ////bool zh_SR2  = ( wwz_score < 0.7 && wwz_score > 0.0 && zh_score > 0.90 );
          //bool original_SRs = ( wwz_SR1 || wwz_SR2 || zh_SR1 || zh_SR2 );
          //// Define new SRs here
	  //bool wwz_SR3 = ( !original_SRs && (zh_score > 0.5 && wwz_score > 0.35) );
	  //bool wwz_SR4 = ( !(original_SRs || wwz_SR3) && (zh_score > 0.85 && wwz_score > -0.5) );
          //bool zh_SR3 = ( !(original_SRs || wwz_SR3 || wwz_SR4 ) && ( wwz_score > 0.8 ) );

          // These are the NEW SRs
          //bool wwz_SR1 = ( zh_score > 0.8 && wwz_score > 0.5 );
          //bool wwz_SR2 = ( !(wwz_SR1) && zh_score > 0.8 && wwz_score > -1.0 && wwz_score < 0.5 );
          //bool zh_SR1 =  ( !(wwz_SR1 || wwz_SR2) && zh_score > -0.5 && zh_score < 0.8 && wwz_score > 0.8 );
          //bool zh_SR2 =  ( !(wwz_SR1 || wwz_SR2 || zh_SR1 ) && zh_score < -0.5 && wwz_score > 0.5 );
          //bool wwz_SR3 = ( !(wwz_SR1 || wwz_SR2 || zh_SR1 || zh_SR2) && zh_score > 0.0 && zh_score < 0.8 && wwz_score < 0.0 );
          //bool wwz_SR4 = ( !(wwz_SR1 || wwz_SR2 || zh_SR1 || zh_SR2 || wwz_SR3) && zh_score > 0.0 && zh_score < 0.8 && wwz_score > 0.0 && wwz_score < 0.8 );
          //bool zh_SR3  = ( !(wwz_SR1 || wwz_SR2 || zh_SR1 || zh_SR2 || wwz_SR3 || wwz_SR4) && zh_score < 0.0 && wwz_score > 0.0 );

          // SRs for BDT v5 
          bool wwz_SR1 = ( wwz_score > 0.8 && zh_score < -0.6 );
          bool wwz_SR2 = ( wwz_score > 0.6 && zh_score < -0.6 && !(wwz_SR1) );
          bool zh_SR1  = ( wwz_score > 0.8 && zh_score > 0.9 && !(wwz_SR1 || wwz_SR2) );
          bool zh_SR2  = ( wwz_score > 0.5 && zh_score > 0.9 && !(wwz_SR1 || wwz_SR2 || zh_SR1) );
          bool wwz_SR3 = ( wwz_score > 0.0 && zh_score < -0.8 && !(wwz_SR1 || wwz_SR2 || zh_SR1 || zh_SR2) );
          bool wwz_SR4 = ( wwz_score > 0.0 && !(wwz_SR1 || wwz_SR2 || wwz_SR3 || zh_SR1 || zh_SR2) );
          bool zh_SR3 = ( !(wwz_SR1 || wwz_SR2 || wwz_SR3 || zh_SR1 || zh_SR2 || wwz_SR4) );
     ...
     //histBDT_BKG->SetMarkerColorAlpha(kGreen,0.5);
     histBDT_BKG->SetMarkerColor(kGreen);
     ...
     if (!makeSRs){
         ...
         cs3->cd();
         TLine* SR1_L1 = new TLine(0.6,-0.6,1.0,-0.6);
         TLine* SR1_L2 = new TLine(0.8,-1.0,0.8,-0.6);
         TLine* SR2_L1 = new TLine(0.6,-1.0,0.6,-0.6);
         TLine* SR3_L1 = new TLine(0.5,0.9,1.0,0.9);
         TLine* SR3_L2 = new TLine(0.8,0.9,0.8,1.0);
         TLine* SR4_L1 = new TLine(0.5,0.9,0.5,1.0);
         TLine* SR5_L1 = new TLine(0.0,-0.8,0.6,-0.8);
         TLine* SR6_L1 = new TLine(0.0,-1.0,0.0,1.0);

         TPaveText *SR1 = new TPaveText(0.85,-0.75,0.95,-0.85,"");
         SR1->SetTextSize(0.02);
         SR1->SetFillColor(0);
         SR1->AddText("SR1");
	 TPaveText *SR2 = new TPaveText(0.65,-0.75,0.75,-0.85,"");
         SR2->SetTextSize(0.02);
         SR2->SetFillColor(0);
         SR2->AddText("SR2");
	 TPaveText *SR3 = new TPaveText(0.85,0.95,0.95,1.05,"");
         SR3->SetTextSize(0.02);
         SR3->SetFillColor(0);
         SR3->AddText("SR3"); 
         TPaveText *SR4 = new TPaveText(0.55,0.95,0.65,1.05,"");
         SR4->SetTextSize(0.02);
         SR4->SetFillColor(0);
         SR4->AddText("SR4");
	 TPaveText *SR5 = new TPaveText(0.2,-0.95,0.4,-0.85,"");
         SR5->SetTextSize(0.02);
         SR5->SetFillColor(0);
         SR5->AddText("SR5");
         TPaveText *SR6 = new TPaveText(0.3,-0.1,0.5,0.1,"");
         SR6->SetTextSize(0.02);
         SR6->SetFillColor(0);
         SR6->AddText("SR6");
         TPaveText *SR7 = new TPaveText(-0.6,-0.1,-0.4,0.1,"");
	 ...
	 std::cout << "Printing yields in Signal Regions" << std::endl;
         ...
         std::cout << "---------------------------------------------" << std::endl;
         std::cout << "WWZ TR Events = " << n_wwz_tot << std::endl;
         std::cout << "WWZ SR Events = " << c1_wwz+c2_wwz+c3_wwz+c4_wwz+c5_wwz+c6_wwz+c7_wwz << std::endl;
         std::cout << "<><><><><><><><><><><><><><><><><><><><" << std::endl;
         std::cout << "ZH TR Events = " << n_zh_tot << std::endl;
         std::cout << "ZH SR Events = " << c1_zh+c2_zh+c3_zh+c4_zh+c5_zh+c6_zh+c7_zh << std::endl;
         std::cout << "<><><><><><><><><><><><><><><><><><><><" << std::endl;
         std::cout << "BKG TR Events = " << n_bkg_tot << std::endl;
         std::cout << "BKG SR Events = " << c1_bkg+c2_bkg+c3_bkg+c4_bkg+c5_bkg+c6_bkg+c7_bkg << std::endl;
         std::cout << "<><><><><><><><><><><><><><><><><><><><" << std::endl;
         
         TH1F* h_SR_wwz   = new TH1F("MVA SR Bins (WWZ)","MVA SR Bins (WWZ)",7,0,7);
         TH1F* h_SR_zh    = new TH1F("MVA SR Bins (ZH)","MVA SR Bins (ZH)",7,0,7);
         TH1F* h_SR_zz    = new TH1F("MVA SR Bins (ZZ)","MVA SR Bins (ZZ)",7,0,7);
         TH1F* h_SR_ttz   = new TH1F("MVA SR Bins (ttZ)","MVA SR Bins (ttZ)",7,0,7);
         TH1F* h_SR_twz   = new TH1F("MVA SR Bins (tWZ)","MVA SR Bins (Higgs)",7,0,7);
         TH1F* h_SR_wz    = new TH1F("MVA SR Bins (WZ)","MVA SR Bins (WZ)",7,0,7);
         TH1F* h_SR_other = new TH1F("MVA SR Bins (Other)","MVA SR Bins (Other)",7,0,7);
         TH1F* h_SR_vvv   = new TH1F("MVA SR Bins (VVV)","MVA SR Bins (VVV)",7,0,7);
         ...
         THStack *hs_SR_bkg = new THStack("hs_SR_bkg","Signal Regions (Same Flavor Channel);SR Bin;Events");  
         ...
         TLegend* legendSR = new TLegend(0.60,0.60,0.70,0.90,"","NDC");
	 ...
	 //hs_SR_bkg->Draw("same");
	 legendSR->Draw();
	 cSR->Print("MVAPlots/SRBin_Yields_SF.png");
     ...
     if (makeDatacard){
         ...
         ofstream datacard;
         datacard.open("datacard_mva_SF_"+tag+".txt");
         ...
         datacard << "observation  " << c1_wwz+c1_zh+c1_bkg << " " << c2_wwz+c2_zh+c2_bkg << " " << c3_wwz+c3_zh+c3_bkg << " " << c4_wwz+c4_zh+c4_bkg << " " << c5_wwz+c5_zh+c5_bkg << " " << c6_wwz+c6_zh+c6_bkg << " " << c7_wwz+c7_zh+c7_bkg << " " << "\n";
         datacard << "------------------------------------------------------------------------------" << "\n";
         ...
         datacard << "rate         " << c1_zh << " " << c1_wwz << " " << c1_bkg << " " << c2_zh << " " << c2_wwz << " " << c2_bkg << " " << c3_zh << " " << c3_wwz << " " << c3_bkg << " " << c4_zh << " " << c4_wwz << " " << c4_bkg << " " << c5_zh << " " << c5_wwz << " " << c5_bkg << " " << c6_zh << " " << c6_wwz << " " << c6_bkg << " " << c7_zh << " " << c7_wwz << " " << c7_bkg << " " << "\n";
         datacard << "------------------------------------------------------------------------------" << "\n";
     ...
     if (makeLatex){
         ...
         ofstream texfile;
	 texfile.open("yields_mva_SF_"+tag+".tex");
	 ...
	 texfile << "\\hline\\hline" << "\n";
	 texfile << "  \\end{tabular}}" << "\n";
         texfile << "  \\caption{Yields in the 4-lepton Same-Flavor MVA signal regions}" << "\n";
